...
var flow_path = 'C:/tagui/flows/samples';
...
if (download_path.indexOf(':')>0) download_path = download_path.replace(/\//g,'\\').replace(/\\/g,'\\');
chrome_step('Page.setDownloadBehavior',{behavior: 'allow', downloadPath: download_path});
chrome_step('Page.navigate',{url: 'https://www.speedtest.net/result/10984636644'}); sleep(1000);
techo('https://www.speedtest.net/result/10984636644' + ' - ' + chrome.getTitle());});
...
casper.then(function() {{techo('hover C:/tagui/flows/samples/ping.png'); var fs = require('fs');
if (!sikuli_step('hover C:/tagui/flows/samples/ping.png')) if (!fs.exists('C:/tagui/flows/samples/ping.png') && !false)
this.echo('ERROR - cannot find image file C:/tagui/flows/samples/ping.png').exit(); else
this.echo('ERROR - cannot find C:/tagui/flows/samples/ping.png on screen').exit(); this.wait(0);}});
...
casper.then(function() {{techo('dclick ('+x+','+y+')'); var fs = require('fs');
if (!sikuli_step('dclick ('+x+','+y+')')) if (!fs.exists('('+x+','+y+')') && !false)
this.echo('ERROR - cannot find image file ('+x+','+y+')').exit(); else
this.echo('ERROR - cannot find ('+x+','+y+') on screen').exit(); this.wait(0);}});

//COPY TO CLIPBOARD
{

casper.then(function() {{techo('keyboard [ctrl]c'); var fs = require('fs');
if (!sikuli_step('keyboard [ctrl]c')) if (!fs.exists('[ctrl]c') && !false)
this.echo('ERROR - cannot find image file [ctrl]c').exit(); else
this.echo('ERROR - cannot find [ctrl]c on screen').exit(); this.wait(0);}});
...
casper.then(function() {this.echo('Ping: '+var_ping+'ms');});

casper.then(function() {this.echo('Downspeed: '+var_down+'Mbps');});

casper.then(function() {this.echo('Upspeed: '+var_up+'Mbps');});
...
casper.then(function() {if ((var_down > 500))
{ // start of code block

casper.then(function() {this.echo('you are a fast guy!');});
...
casper.then(function() {this.echo('you are too slow, pls go home!');});
