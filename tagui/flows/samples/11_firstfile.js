...
chrome_step('Page.navigate',{url: 'https://www.google.de'}); sleep(1000);
techo('https://www.google.de' + ' - ' + chrome.getTitle() + '\n');});

}

//SELECT GOOGLE´S TEXTBOX, gLFyf gsfi IS SELECTOR CLASS
{

casper.then(function() {{techo('type gLFyf gsfi as iloveminnie4eva');
casper.waitFor(function check() {return check_tx('gLFyf gsfi');},
function then() {chrome.sendKeys(tx('gLFyf gsfi'),'iloveminnie4eva');},
function timeout() {this.echo('ERROR - cannot find gLFyf gsfi').exit();});}});

}

//CLICK BUTTON, btnK IS SELECTOR NAME
{

casper.then(function() {{techo('click btnK');
casper.waitFor(function check() {return check_tx('btnK');},
function then() {chrome.click(tx('btnK'));},
function timeout() {this.echo('ERROR - cannot find btnK').exit();});}});
...
//SAVE SCREENSHOT OF WEBSITE IN WORKFLOW´S FOLDER
{

casper.then(function() {{techo('snap page to top_result.png');
chrome.capture('C:/tagui/flows/samples/top_result.png');}});
