...
casper.then(function() {{techo('type q as latest movies[enter]');
casper.waitFor(function check() {return check_tx('q');},
function then() {chrome.sendKeys(tx('q'),'latest movies',{keepFocus: true});
chrome.sendKeys(tx('q'),casper.page.event.key.Enter,{keepFocus: true});
chrome.sendKeys(tx('q'),'',{keepFocus: true});},
...
function timeout() {this.echo('ERROR - cannot find (//*[@class="g"])[1]//a').exit();});}});
